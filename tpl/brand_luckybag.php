...
<meta charset="UTF-8">
<title></title>
<link rel="stylesheet" type="text/css" href="./css/brand_luckybag.css">
</head>
...
<header>
    <form action="brand_luckybag.php" method="post">
    <table>
        <tr>
...
<div id="paging">
    ...
    if (isset($_GET['page_id'])  && $_GET['page_id']!= 1 ){ ?>
            <a href="./brand_luckybag.php?page_id=<?php echo $_GET['page_id']-1; ?>"><?php echo '前へ'; ?></a>
    <?php }else{ ?>
    ...
    <?php }
     for ($i = 1; $i <= $max_page;$i++){
        if ($i == $now){
            ...
            <a href="./brand_luckybag.php?page_id=<?php echo $i; ?>"><?php echo $i;?></a>
        <?php    }
     }
     if (isset($_GET['page_id']) && $_GET['page_id']  == $max_page){ ?>
         <span><?php echo '次へ'; ?></span>
    <?php }elseif($now == 1){ ?>
        <a href="./brand_luckybag.php?page_id=2"><?php echo '次へ'; ?></a>

    <?php }else { ?>
        <a href="./brand_luckybag.php?page_id=<?php echo $_GET['page_id']+1; ?>"><?php echo '次へ'; ?></a>
    <?php } ?>
    <?php } ?>
